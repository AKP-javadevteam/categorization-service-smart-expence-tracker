...
public class CategorizationService {
    ...
    public Transaction categorize(Transaction transaction) {
        ...
        for (Rule rule : rules) {
            ...
            if (transaction.getMerchant().toLowerCase().contains(rule.getDigester()
                    || ((transaction.getDescription() != null) && transaction.getDescription().toLowerCase().contains(rule.getDigester().toLowerCase()))) {
